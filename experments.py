...
for service, data in DATABASE.items():
    if 'request' in data:
        ...
        plt.scatter(bandwidth, latency, label=service,
                    s=data['request']['vistes'] * 10)  # Adjust the size of the point based on the number of requests
...
# Step 1: Define a graph representing the network using NetworkX
G = nx.Graph()

# Add edges to the graph based on the NETWORK_TREE
for path in NETWORK_TREE:
    for i in range(len(path) - 1):
        G.add_edge(path[i].__name__, path[i+1].__name__)

# Step 2: Apply the Louvain algorithm to detect communities in the graph
partition = community.best_partition(G)

# Step 3: Map the detected communities to your network tree
communities = {}
...
# Plot a bar chart to visualize the detected communities
community_sizes = [len(nodes) for nodes in communities.values()]
community_labels = [f"Community {comm_id + 1}" for comm_id in communities.keys()]
